...
mod tests {
    ...
    fn test_move() {
        ...
        actions_system.move(Direction::UpRight(()));

        let moves: Moves = world.read_model(caller);

        let up_right_dir_felt: felt252 = Direction::UpRight(()).into();
        assert(moves.remaining == initial_moves.remaining - 1, 'moves is wrong');
        assert(moves.last_direction.into() == up_right_dir_felt, 'last direction is wrong');

        let new_position: Position = world.read_model(caller);
        if (initial_position.vec.x + 1 < 7)
            && (initial_position.vec.y > 0)
            && ((initial_position.vec.x + 1 + initial_position.vec.y - 1) % 2 == 0) {
            assert!(new_position.vec.x == initial_position.vec.x + 1, "position x is wrong");
            assert!(new_position.vec.y == initial_position.vec.y - 1, "position y is wrong");
        }
    }
}
